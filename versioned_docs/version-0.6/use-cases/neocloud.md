...
A *neocloud* is a GPU-centric cloud built on a fleet of high-density NVMe storage servers spread across several US regions. The goal is an AWS S3-like service (the same API, the same "11 nines" durability promise, the same regional model) running on your own hardware at a fraction of the cost.
...
A minimal DC of 3 racks x 3 servers = **9 hosts cannot run `RS(8,4)`** (needs 12). Either shrink the scheme (`RS(4,2)`, `RS(6,3)`), grow the DC, or accept `placement_policy = "pack"` (which drops the spread guarantee, not recommended for a durability tier). For a comfortable `RS(8,4)` tier, target **>= 4 racks x >= 4 servers = 16 hosts per pool.**

### Rule 2: spread so a whole rack/DC loss stays within the parity budget

If a stripe of `N` shards is spread evenly across `D` domains at some level (e.g. racks), each domain holds at most `ceil(N/D)` shards. To survive losing `g` whole domains **and** still tolerate `h` additional independent shard failures during the rebuild:
...
| Racks (D) | Shards/rack ceil(N/D) | Survives 1 rack loss? | Extra failures tolerated while rack is down (h) |
|---:|:---:|:---:|:---:|
...
|---|:---:|---|---|
| Data shards | `K` | up efficiency, up rebuild I/O, needs more hosts | 4 - 16 |
| Parity shards | `M` | up fault tolerance, down efficiency | 2 - 4 |
| Total shards | `N = K + M` | hosts required per stripe | `K + M` |
| Effective shard failure rate | `lambda` | down durability; `lambda ~= AFR_drive + AFR_host` /year | 0.02 - 0.08 |
| Repair rate | `mu = 1 / MTTR` | up durability **steeply** (enters as `mu^M`) | MTTR 2 - 24 h |
| Geo-replicas | `C` | up region-loss protection, divides efficiency | 1 - 3 |
| Racks per pool | `D_rack` | governs rack-loss survival via Rule 2 | >= 4 |
| Placement policy | n/a | `strict` guarantees spread; `pack` favors availability | `strict` for durability tiers |

`C(N, M+1)` is the binomial coefficient ("number of ways `M+1` of `N` shards can fail").

### Worked example: `RS(8,4)`, host failure domain

Inputs: `lambda = 0.04/yr` (~ 1% drive AFR + 3% host AFR), `MTTR = 8 h`, so `mu = 8760/8 = 1095/yr`.

```
mu^M           = 1095^4              ~= 1.44e12
C(12,5)        = 792
(M+1)          = 5
lambda^(M+1)   = 0.04^5              ~= 1.02e-7
...
You drive each correlated term below target with the **spread constraint (Rule 2)** and with **cross-region replication**, not by piling on parity. And because `MTTDL` is proportional to `mu^M`, *halving rebuild time on `RS(8,4)` raises durability ~16x*: faster [healing](../operations/healing) and LRC local repair buy more durability than extra parity, at zero space cost.
:::
...
```
                K
   eta  =  ---------------          (usable / raw capacity)
            C * (K + M)

   raw_per_usable_byte = C * (K + M) / K = C * (1 + M/K)
```

where `C` is the number of full geo-replicas (`C = 1` single region, `C = 2` one DR copy, and so on).

| Scheme | N = K+M | Local eta | Overhead | eta with `C=2` |
|---|---:|---:|---:|---:|
| `RS(4,2)` | 6 | 66.7% | 1.50x | 33.3% |
| `RS(8,3)` | 11 | 72.7% | 1.38x | 36.4% |
| **`RS(8,4)`** (default) | 12 | 66.7% | 1.50x | 33.3% |
| `RS(12,4)` | 16 | 75.0% | 1.33x | 37.5% |
| `RS(16,4)` | 20 | 80.0% | 1.25x | 40.0% |
| `LRC(12,3,3)` | 18 | 66.7% | 1.50x | n/a |
...
# High-Performance default (>= 4 racks x >= 4 servers per region)
...
- Size pools to **`K + M` hosts minimum** and **>= 4 racks** so a rack loss stays within the parity budget (Rule 2: `g * ceil(N/D) + h <= M`).
- Tune the trade-off with the durability formula `MTTDL ~= mu^M / (C(N,M+1) * (M+1) * lambda^(M+1))` and the efficiency formula `eta = K / (C * (K+M))`.
- Remember the levers that cost no space: **faster rebuild (`mu`)** and **wider spread**, not more parity.
