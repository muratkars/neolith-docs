...
- **Tiered objects read back at their key (GH #347)**: an object a lifecycle `Transition` moved to a remote S3-compatible tier is served again by a plain `GET`, `HEAD`, range `GET` and batch `GET`; the range of an uncompressed, unencrypted object is fetched from the tier as a range. The transition now writes the pointer record before it drops the local copy, and a transition to a GCS or Azure target is refused instead of being reported as done with no remote copy. Tier failures surface as `503 ServiceUnavailable` (unreachable tier) or `500 InternalError` (tier not configured on this node). See [Lifecycle Rules](./s3-api/lifecycle.md#transitions-and-cloud-tiering).
