...
- **Single-node `storage.scheme` now defaults to `journal`**: the group-commit journal + erasure-coded-stripe write path is no longer opt-in for single-node deployments that don't set `scheme` explicitly - it's the target architecture, and the cutover gate (S3 conformance, dependent-subsystem coverage, crash and simulated-power-loss durability, cargo-mutants/miri, and a real Linux/NVMe throughput re-baseline that caught and fixed a mixed-workload regression) is now fully closed. Multi-node clusters are unaffected and still default to `replicated` until the placement work that removes journal cluster mode's storage overhead lands; setting `scheme` explicitly (either value, any cluster size) is always honored. See [Configuration](/docs/operations/configuration#storage).
