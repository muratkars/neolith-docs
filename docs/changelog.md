...
- **Event notifications carry a `sequencer` (GH #364)**: every event record's `object` carries the write's hybrid logical clock stamp as a fixed-width hex string, so a subscriber orders two events for one key by a string compare and discards the older one, the same way the S3 field of that name is used; absent on single-node deployments, which stamp no clock. See [Event Notifications](./s3-api/notifications.md#ordering-events-with-sequencer).
- **Bucket configuration converges across nodes (GH #363)**: `CreateBucket`, `DeleteBucket`, `PutBucketVersioning`, lifecycle, CORS, policy, website, tagging and notification configuration are replicated to every node when written and reconciled last-writer-wins per bucket and kind over the heartbeat, so a bucket created or a versioning status set at one node holds at every node, and a node that was down for the write converges on its own. A removed bucket is kept as a tombstone and is never handed back by a node that still had it. Before this, every bucket configuration lived only on the node that served the request. See [Cluster Architecture](./architecture/cluster.md#bucket-configuration).
- **Tier-out of a versioned object (GH #368)**: the current version of a versioning-enabled bucket transitions to a remote tier like any other object, releasing its per-version bytes once the tier holds them, and reads through the record both at its key and by `?versionId=`; each version has its own copy in the tier (`<bucket>/<key>/.v/<version-id>`), and permanently deleting a version releases it. Before this the transition's rewrite check looked at a file the version never had, and a GET by version id of a tiered version answered NoSuchKey. See [Lifecycle Rules](./s3-api/lifecycle.md#reading-a-tiered-object).
