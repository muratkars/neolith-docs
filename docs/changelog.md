...
- **Overwrites correctly supersede the journal**: overwriting a journaled object with one that routes to the meta store (a larger object, a copy destination, or a write to a now-versioned bucket) previously left the old journaled copy live, so reads kept returning the stale object. The write now tombstones the superseded journal entry, and a delete of a journaled object also removes any tagging sidecar so a re-created key does not inherit old tags.
