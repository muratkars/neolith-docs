...
- **Checksum algorithm agreement is enforced at the heartbeat**: each node advertises the checksum algorithm it hashes with in the gossiped topology, a peer on a different algorithm is fenced offline at the first heartbeat that reaches it and stays fenced until it matches again, the inter-node RPC layer covers the window and older peers, and the advertised algorithm shows in the admin node view. See [Configuration](./operations/configuration.md#journal-checksum-algorithm).
